        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant overflow-x-auto">
          <table className="w-full text-left min-w-[900px]">
            ...
            <tbody className="divide-y divide-outline-variant/10 text-sm">
              {loading ? <tr><td colSpan="5" className="p-10 text-center">Đang tải...</td></tr> : result.content.length === 0 ? <tr><td colSpan="5" className="p-10 text-center text-on-surface-variant">Không tìm thấy người dùng.</td></tr> : result.content.map((user) => {
                ...
                return <tr key={user.id} className="hover:bg-surface-container-low/50">
                  ...
                  <td className="px-6 py-4 font-bold">{user.role}</td><td className="px-6 py-4 text-on-surface-variant">{user.createdAt ? new Date(user.createdAt).toLocaleDateString('vi-VN') : '—'}</td>
